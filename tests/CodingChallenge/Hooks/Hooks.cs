...
namespace CodingChallenge.Hooks
{
    ...
    public class Hooks
    {
        ...
        public static async Task BeforeTestRunAsync() 
        {
            //Fecth Url
            LoadPlaywrightSettings();
            var _baseUrl = _playwrightSettings!.BaseUrl;
        ...
        {
            // Skip browser set up if its a API test only
            if(_scenarioContext!.ScenarioInfo.Tags.Contains("ApiTest")) { return; }

            //Initalize Browser with config settings
            ...
            _objectContainer.RegisterInstanceAs<PlaywrightSettings>(_playwrightSettings!);
        }
        ...
        private async Task<IBrowser> InitializeBrowserAsync()
        {
            ...
            switch (_playwrightSettings!.Browser!.ToLower())
            {
                ...
                default:
                    throw new ArgumentException("Invalid Browser type selected, update appsetting.json with Valid Browser");
            }
        }
